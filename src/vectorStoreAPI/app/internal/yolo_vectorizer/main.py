...
class YoloDetector:
    ...
    def __init__(self, video_vectorizer: AbstractVideoVectorizer,
                 model_path: str = "./models/yolo.pt", device: str = "cuda", inference_batch_size: int = 16):
    ...
    def process_video_frames(self, video_path):
            ...
            if (frame_num % self.batch_size == 0) | (frame_num == total_frames):
                ...
                for idx, segment in enumerate(results):
                    try:
                        ...
                        if segment.boxes.conf.cpu()[0] < 0.93:
                            continue
                    except IndexError:
                        write_segment = False
                        continue
        ...
        detections = list(filter(lambda x: x.length > 80, detections))
        cap.release()
